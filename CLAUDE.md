...
### Bash
- Используй одиночные команды. Избегай `&&` и `|` без явной необходимости — компаунды ломают permission-матчинг и цепляют approval-меню на безопасных операциях.
- Не префиксуй каждую команду `cd /полный/путь/`. Ты остаёшься в правильной директории между командами.
- Если нужно несколько read-only действий подряд (`ls`, `cat`, `wc`, `head`) — запускай их по одной, каждая попадёт под allow-list.

### Экономия approval'ов
- Время Дениса дороже эстетики терминала. Оптимизируй число approval-меню, не число строк.
- Перед ask-операцией (`git push`, `git remote add`, install) — сначала краткий отчёт Денису в одном сообщении, потом один запрос approval. Не дёргай по чуть-чуть.

### Отчёты Денису
- После каждого файла в `/research/` — короткий отчёт (3–5 буллетов).
- Перед commit `/research/LANDING_BRIEF.md` — стоп, отчёт Денису, ждать ревью от архитектора.
- После deploy — полный отчёт.
- Мелких «думаю над этим» сообщений — не нужно.

### Полномочия
- Имена файлов, структура кода, рефакторинг — твоё одно.
- Scope, новые фичи, технологии вне `/docs/STACK.md` — эскалация к архитектору через Дениса.
- Визуальная интерпретация бренда — эскалация. Правило про **isometric cube cropped by canvas** — non-negotiable, никогда не рендери polygon/hexagon под видом «творческой интерпретации».

### В разговоре с Денисом
- Не клади bash-команду как ответ. Сначала кратко переведи состояние на русский, потом по желанию покажи команду.
- Каждый технический термин при первом появлении в сессии — кратко в скобках на русском.

### В неопределённости
- Предпочитай одно продуманное действие десяти «можно так?».
- Если ask-операция явно безопасна — всё равно проси approval, но сразу предлагай ответ: «готов запушить в origin/main, ok?».
- Если пахнет scope-creep — стоп, эскалируй через Дениса, не решай в одиночку.
